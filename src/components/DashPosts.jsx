...
const DashPosts = () => {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:4000/api/v1/post/getposts?userId=${currentUser?.rest?._id}&startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:4000/api/v1/post/deletepost/${postIdToDelete}/${currentUser?.rest?._id}`,
        {
          method: "DELETE",
        }
      );
